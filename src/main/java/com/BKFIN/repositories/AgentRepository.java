...
public interface AgentRepository extends CrudRepository<Agent,Long> {
	...
	@Query(" select u from Agent u " + " where u.idAgent = ?1")
    Agent findUserWithID(Long id);
	
	@Query(" select u from Agent u " + " where u.email = ?1")
    Agent findUser (String username);
	
	@Query(" select u from Agent u  where u.classification = ?1")
	List<Agent> findAgentClass (ClassEnum c);
}
